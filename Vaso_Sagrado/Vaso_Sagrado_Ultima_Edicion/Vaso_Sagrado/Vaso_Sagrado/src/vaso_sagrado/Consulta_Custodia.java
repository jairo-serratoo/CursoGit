...
public class Consulta_Custodia extends javax.swing.JInternalFrame {
String ar1[]={"Nombre","Altura","Coste","Existencia"};
String ar2[][]=new String [1][4];
    public Consulta_Custodia() {
        initComponents();
        ar2=llenarArreglo2();
     ...
     public String[][] llenarArreglo2(){
        ...
        try{
            ...
            ResultSet rs=st.executeQuery("select * from Custodias");
            if(rs.last());
            int filas=rs.getRow();
            temp=new String[filas][4];
            rs.first();
            for(int i = 0; i< temp.length; i++){
                //temp[i][0]=rs.getString("IdClass");
               // temp[i][0]=rs.getString("idCustodia");
